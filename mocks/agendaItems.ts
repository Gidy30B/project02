...
function getFutureDates(numberOfDays: number): string[] {
  const array: string[] = [];
...
function getPastDate(numberOfDays: number): string {
  return new Date(Date.now() - 864e5 * numberOfDays).toISOString().split('T')[0];
}

export const agendaItems: AgendaSection[] = [
  {
    title: dates[0],
...
export function getMarkedDates(): MarkedDates {
